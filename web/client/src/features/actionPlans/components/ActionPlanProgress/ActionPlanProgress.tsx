...
export function ActionPlanProgress({ actionPlans }: ActionPlanProgressProps) {
  ...
  const totalCompletionRate = actionPlans.length > 0 ? Math.round((totalCompleted / actionPlans.length) * 100) : 0
    ...
    <div className="space-y-6">
      <Card className="bg-gray-950 border-gray-800 text-white">
        <CardHeader className="border-b border-gray-800">
          ...
          <CardDescription className="text-gray-400">Completion rate of all action plans</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-gray-400">
                {totalCompleted}/{actionPlans.length} Completed ({totalCompletionRate}%)
              </span>
            </div>
            <Progress value={totalCompletionRate} className="h-2 bg-gray-800" indicatorClassName="bg-rose-500" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gray-950 border-gray-800 text-white">
        <CardHeader className="border-b border-gray-800">
          ...
          <CardDescription className="text-gray-400">Action plan completion rate by category</CardDescription>
        </CardHeader>
        <CardContent className="pt-4">
          <div className="space-y-6">
            {Object.entries(categories).map(([category, data]) => {
              ...
              return (
                <div key={category} className="space-y-2">
                  ...
                  <Progress value={completionRate} className="h-2 bg-gray-800" indicatorClassName={`${data.color}`} />
                </div>
              )
            })}
          </div>
      ...
      <Card className="bg-gray-950 border-gray-800 text-white">
        <CardHeader className="border-b border-gray-800">
          ...
          <CardDescription className="text-gray-400">Action plan completion rate by priority</CardDescription>
        </CardHeader>
        <CardContent className="pt-4">
          <div className="space-y-6">
